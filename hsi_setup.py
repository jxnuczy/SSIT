...
def train_options(parser):
    ...
    parser.add_argument('--batchSize', '-b', type=int,
                        default=8, help='training batch size. default=16')
    ...
    # /home/lch/Czy/SERT-master/icvl_noise_50/512_mix'
    # /home/lch/Czy/SERT-master/icvl_train.db
    parser.add_argument('--resumePath', '-rp', type=str,
                        default=None, help='checkpoint to use.')
    parser.add_argument('--test-dir', type=str,
                        default='/home/lch/Czy/SERT-master/icvl_noise_50/512_mix', help='The path of test HSIs')
    parser.add_argument('--dataroot', '-d', type=str,
                        default='/home/lch/Czy/SERT-master/icvl_train.db', help='data root')
...
#        print(self.net.flops([64,64]))
    ...
    def train(self, train_loader,val):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if not self.opt.no_log:
                writer = SummaryWriter("/home/lch/Czy/SERT-master/log")
                writer.add_scalar('train_psnr',avg_psnr,global_step=self.iteration)
                writer.add_scalar('train_loss', loss_data, global_step=self.iteration)
                writer.add_scalar('train_avg_loss', avg_loss, global_step=self.iteration)
                writer.close()
        ...
        if not self.opt.no_log:
            ...
            writer = SummaryWriter("/home/lch/Czy/SERT-master/log")
            writer.add_scalar('train_loss_epoch',avg_loss,global_step=self.epoch)
            writer.close()
    ...
    def test(self, valid_loader, filen):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(valid_loader):
                ...
                # result = outputs.squeeze().cpu().detach().asnumpy()
                ...
                # inputs = outputs.squeeze().cpu().detach().numpy()
                # result = outputs.squeeze().cpu().detach().numpy()
                result1 = targets.squeeze().cpu().detach().numpy()
                result2 = inputs.squeeze().cpu().detach().numpy()
                ...
                scio.savemat('/home/lch/Czy/SERT-master/test-mat/ICVL_mix/'+self.opt.arch+'gt.mat',{'result':result1})
                scio.savemat('/home/lch/Czy/SERT-master/test-mat/ICVL_mix/' + self.opt.arch + 'noisy.mat', {'result': result2})
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(valid_loader):
                ...
                temp = (np.sum(result*img, 0) + np.spacing(1)) \
                    /(np.sqrt(np.sum(result**2, 0) + np.spacing(1))) \
                    /(np.sqrt(np.sum(img**2, 0) + np.spacing(1)))
                #print(np.arccos(temp)*180/np.pi)20535MiB
                sam = np.mean(np.arccos(temp))*180/np.pi
                SAM.append(sam)
    ...
    def validate(self, valid_loader, name,patch_size=64):
        ...
        if not self.opt.no_log:
            ...
            writer = SummaryWriter("/home/lch/Czy/SERT-master/log")
            writer.add_scalar('val_loss_epoch', avg_loss, global_step=self.epoch)
            writer.add_scalar('val_psnr_epoch', avg_psnr, global_step=self.epoch)
            writer.add_scalar('val_sam_epoch', avg_sam, global_step=self.epoch)
